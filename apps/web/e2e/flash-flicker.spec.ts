 ...
 * CORRECT behavior. The fix lands on `feat/ui-redesign`, not `dev`, so parts
 * of this are EXPECTED TO FAIL here — see the report for exactly which parts
 * and why. Not weakened to pass against the current code.
 */
...
test.describe("chat switching: flash, flicker, and content jumping", () => {
  test("revisiting an already-loaded chat this session shows no blank loading state, and rendered messages never surface raw markdown/LaTeX", async ({
    ...
    await expect(page.locator(".turn-assistant .markdown").last()).toContainText("CPU burst", { timeout: 15_000 });

    await page.getByRole("link", { name: FLICKER_CHAT_B }).click();
    await expect(page.locator(".turn-assistant .markdown").last()).toContainText("Shortest Job First", {
      timeout: 15_000,
    });
    ...
    const assistantText = await page.locator(".turn-assistant .markdown").last().innerText();
    expect
