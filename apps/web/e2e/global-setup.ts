 ...
 * explicitly before importing `@mola/db` (which reads `DATABASE_URL` at
 * import time).
 */
...
async function main() {
  const { db, users, chats, messages, compactionBoundaries } = await import("@mola/db");
  ...
  for (const fx of flickerFixtures) {
    ...
    const [chat] = await db
      .insert(chats)
      .values({ userId: alice.id, courseId: null, title: fx.title })
      .returning();
    const base = Date.now();
    await db.insert(messages).values([
      {
        userId: alice.id, chatId: chat!.id, role: "user", content: fx.user,
        createdAt: new Date(base),
      },
      {
        userId: alice.id, chatId: chat!.id, role: "assistant", content: fx.assistant,
  ...
  // COMPACT_THRESHOLD=16, KEEP_RAW=8 (lib/agent/compaction.ts) — 20 raw turns
  // with a boundary at turn 12 mirrors what maybeCompact would have produced,
  // without paying for a real 20-turn LLM conversation.
  ...
  if (!existingCompactionChat) {
    const [chat] = await db
      .insert(chats)
      .values({ userId: alice.id, courseId: null, title: compactionTitle })
      .returning();
...
await main();
// The `postgres` client keeps a connection pool open, which otherwise leaves
// this process (and, worse, Playwright's runner waiting on it) hanging.
process.exit(0);
